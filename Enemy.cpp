...
void Enemy::MakeBullet(vector<Bullet*> &bullet,SDL_Renderer* screen,Character &spaceship)
{
    ...
    if(type==1.1)
    {
        ...
        p_bullet->LoadTexture("img//BulletThreat.png",screen);
        p_bullet->set_is_move(true);
        if(spaceship.GetRect().x-x_pos>=0) //góc bắn
        {
        ...
        p_bullet->set_x_speed(2);
        p_bullet->set_y_speed(2);
        p_bullet->set_pos(x_pos+width_frame/2-(p_bullet->GetRect().w)/2,y_pos+24);//nơi bắn đạn
        bullet.push_back(p_bullet);
    }
    //đạn tím xoay tròn
    else if(type==2.1)
    {
        ...
        p_bullet2->set_angle(90+angle);//sửa lại thành 50
        rotate_angle();
    ...
    else if(type==6.1)
    {
        if(die>=200 && die<300)
        {
            ...
            {
                delay_shoot_time=100;
                for(int j=0;j<24;j++)//bắn hình thoi
                {
                    for(int i=0;i<4;i++)
                    {
                        Bullet*p_bullet=new Bullet();
            ...
            {
                delay_shoot_time=500;
                for(int j=0;j<36;j++)//bắn xoay 360
                {
                    Bullet*p_bullet=new Bullet();
                    p_bullet->set_is_move(true);
        ...
        else if(die>=300 && die<400)
        {
            ...
            x_speed=0;
            for(int i=0;i<12;i++)//bắn từ 1 vòng tròn trung tâm
            {
                Bullet*p_bullet1=new Bullet();
    ...
    else if(type==7.1)//đạn đuổi hình thoi
    {
