...
int main( int argc, char *argv[] )
{
    ...
    spaceship.set_clip();
    test_menu.LoadTexture("img//bulletheaven.png",g_screen);
    ...
    while(play)
    {
        ...
        if(wave%10==0 && wave!=0)
        {
            ...
            Mix_HaltChannel(2);//dừng phát âm kênh 2
        }
        else
        {
            if(!Mix_Playing(2))
        ...
        if(!GameOver)
        {
            if(paused)
            {
                SDL_WarpMouseInWindow(g_window,spaceship.GetRect().x,spaceship.GetRect().y);//di chuyển chuột đến nơi chỉ định
                while(SDL_PollEvent(&g_event)!=0)
                {
                    if(g_event.type==SDL_QUIT)
                    {
        ...
        else
        {
             ...
             numberofcurrentscore.RenderText(g_screen,SCREEN_WIDTH/2-50,SCREEN_HEIGHT/2 - 115);

             Wave.Set_Text(number_to_string(wave));
             Wave.LoadFromRenderText(paused_font,g_screen);
             Wave.RenderText(g_screen,SCREEN_WIDTH/2-50,SCREEN_HEIGHT/2);

             Rating.Set_Text(RatingBaseOnScore(current_score));
             Rating.LoadFromRenderText(paused_font,g_screen);
             Rating.RenderText(g_screen,SCREEN_WIDTH/2,SCREEN_HEIGHT/2+125);
